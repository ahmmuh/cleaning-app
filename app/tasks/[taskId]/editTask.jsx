...
function EditTask() {
  ...
  const [task, setTask] = useState({
    title: "",
    description: "",
    address: "", // ändrat från location till address
    location: { type: "Point", coordinates: [0, 0] }, // behåll location för backend
    status: "Ej påbörjat",
  });
  ...
  const fetchTask = async () => {
    try {
      ...
      setTask({
        ...foundedTask,
        address: foundedTask.address || "",
        location: foundedTask.location || {
          type: "Point",
          coordinates: [0, 0],
        },
      });
  ...
  const handleSubmit = () => {
    try {
      const updatedTask = {
        title: task.title,
        description: task.description,
        address: task.address, // skickar adress som text
        location: task.location, // behåll GeoJSON
        status: selectStatus,
      };
      updateTaskById(taskId, updatedTask);
      console.log("Uppdaterade uppdrag", updatedTask);
    ...
    <ScrollView style={{ flex: 1 }}>
      <View style={styles.container}>
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput
            style={styles.textInput}
            value={task.address} // ändrat från location
            onChangeText={(text) =>
              setTask((prevTask) => ({
                ...prevTask,
                address: text,
...
const styles = StyleSheet.create({
  ...
  descriptionInput: {
    ...
    minHeight: 150,
    textAlignVertical: "top",
    paddingTop: 12, // använd bara paddingTop
    paddingBottom: 12, // valfritt
