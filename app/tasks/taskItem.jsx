...
function TaskItem({ item }) {
  ...
  // Kontroll: redigering bara om task.unit matchar currentUser.unit
  const canEdit = item?.unit?._id?.toString() === user?.unit?._id?.toString();
    ...
    <View style={styles.container}>
      ...
      {canEdit && (
        <View style={styles.linkContainer}>
          <Link href={`/tasks/${item._id}/editTask`} style={styles.editLink}>
            <FontAwesome name="edit" size={16} color="white" />
            <Text style={styles.editLinkText}> Byt status</Text>
          </Link>
        </View>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  updatedAt: { fontSize: 12, color: "#232020ff" },
  createAt: { fontSize: 12, color: "#232020ff" },
  linkContainer: { marginTop: 12, alignItems: "flex-end" },
  editLink: {
    flexDirection: "row",
    backgroundColor: "rgba(22, 144, 200, 1)",
    paddingVertical: 6,
    paddingHorizontal: 12,
    borderRadius: 6,
    alignItems: "center",
  },
  editLinkText: { color: "#fff", fontWeight: "bold", fontSize: 14 },
});
